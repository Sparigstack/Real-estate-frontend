...
export default function FloorBasedUnits({ setWingStep }) {
    ...
    const submitFloorViseUnitDetails = async (values) => {
        setLoading(true);
        const floorUnitCountsArray = utils.floorUnitCounts.map(floor => ({
        ...
        <>
            ...
            <div className="row p-4">
                <div className='col-md-6 offset-md-3'>
                    ...
                    <Formik initialValues={utils.floorUnitCounts.reduce((acc, floor) => {
                        acc[`unitsEachFloor${floor.floorId}`] = '';
                        return acc;
                    }, {})} validationSchema={floorWiseUnitsValidationSchema(utils.floorUnitCounts)} onSubmit={submitFloorViseUnitDetails}>
                        {({ setFieldValue }) => {
                            const handleAutofill = (value) => {
                                utils.floorUnitCounts?.forEach((floor) => {
                            ...
                            return (
                                <Form className='pt-4 mt-2'>
                                    <div className='d-flex flex-wrap gap-4 justify-content-center'>
                                        {utils.floorUnitCounts?.map((floor, floorIndex) => (
