...
export default function Csv() {
    ...
    const getExportLead = async () => {
        setLoading(true);
        try {
            const result = await getAPIAuthKey(`/exportLeads/${schemeId}?userCapabilities=excel_import_export&flag=1`);
            if (!result) {
                throw new Error('Something went wrong');
            }
            const responseRs = JSON.parse(result);
                        ...
                        <div className="text-center row mt-4 p-2">
                            ...
                            {divVisibleForExport &&
                                <div className="row pt-5 fontwhite">
                                    <div>Your Leads CSV file is now ready for download. Please click the button below to initiate the file download.</div>
